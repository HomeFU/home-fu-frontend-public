        ...
        <div className={style.wrapperRuestBlock}>
                ...
                categorys.map((category, index) => (
                    <div className={style.itemBlock} key={category.id}>
                        ...
                        <div className={ style.counterBlock}>
                            <button className={counterValue[index] == 0 ? style.button_no_drop: style.buttonDecrement} onClick={() => {dispath(decrement(index))}}>
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <span>{counterValue[index]}</span>
                            <button className={style.buttonIncrement} onClick={() => {dispath(increment(index))}}>
            ...
            <div className={style.itemBlock}>
                <div className={style.itemDescription}>
                    ...
                    <span className={style.travelWithAnimal}>Подорожуєте із твариною-помічником?</span>
                </div>
                <div className={ style.counterBlock}>
                    <button className={counterValue[3] == 0 ? style.button_no_drop: style.buttonDecrement} onClick={() => {dispath(decrement(3))}}>
                        <FontAwesomeIcon icon={faMinus} />
                    </button>
                    <span>{counterValue[3]}</span>
                    <button className={style.buttonIncrement} onClick={() => {dispath(increment(3))}}>
                        <FontAwesomeIcon icon={faPlus} />
                    </button>
                </div>
            </div>
